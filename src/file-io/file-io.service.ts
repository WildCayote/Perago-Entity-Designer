...
export class FileIoService {
  ...
  async getProject(id: string, pattern: string = "default") {
    ...
    const project_name = this.toKebabCase(projectData.projectName.toLowerCase());
    ...
    for (const [key, value] of Object.entries(dtos)) {
      zip.file(`${project_name}/src/dtos/${this.toKebabCase(key)}.dto.ts`, value as string);
    }
    ...
    for (const [key, value] of Object.entries(controllers)) {
      ...
      zip.file(`${project_name}/src/modules/${this.toKebabCase(key)}/controllers/index.ts`, this.controllersService.generateBarrel([key]));
    }
    ...
    for (const [key, value] of Object.entries(services)) {
      ...
      zip.file(`${project_name}/src/modules/${this.toKebabCase(key)}/services/index.ts`, this.servicesService.generateBarrel([this.toKebabCase(key)]));
    }
    // zip.file(`${project_name}/src/services/index.ts`, servicesBarrel);

    // for modules
    for (const [key, value] of Object.entries(modules)) {
      zip.file(`${project_name}/src/modules/${this.toKebabCase(key)}/${this.toKebabCase(key)}.module.ts`, value as string);
      zip.file(`${project_name}/src/modules/${this.toKebabCase(key)}/index.ts`, this.modulesService.generateBarrel([this.toKebabCase(key)]));
    }
